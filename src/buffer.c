...
/*   Updated: 2016/12/01 12:39:45 by wwatkins         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "scop.h"

/*	Creates the object buffers of differents types (VBO, VAO, EBO).
*/
void	create_buffers(t_env *env, GLfloat *vertices, GLuint *indices, int mode)
{
	...
	glBufferData(GL_ARRAY_BUFFER, env->data.size_vertices, vertices, mode);
	glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, env->buffer.EBO);
	glBufferData(GL_ELEMENT_ARRAY_BUFFER, env->data.size_indices, indices, mode);
	/* Position attributes */
